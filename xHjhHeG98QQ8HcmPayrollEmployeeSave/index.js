...
exports.handler = async event => {
  ...
  if(body.sheetInitial.employee) {
    let employee = await api.get(`/hcm/payroll/entities/employee/${body.sheetInitial.employee.tableId}`);
  ...
  if((body.sheetContract.customFieldsEmployee) && (body.sheetComplement.issueDotCard)) {
    ...
    if((body.sheetPersona.isDisability === "true") && (body.sheetPersona.isOccupantQuota === "false")) {
     ...
     if((body.sheetInitial.contractType.key === 'Employee') && (body.sheetWorkSchedule.workshift.tableId)){
        try {
            let workshiftResponse = await api.get(`/hcm/payroll/entities/workshift/${body.sheetWorkSchedule.workshift.tableId}`);
    ...
    if(body.sheetDocument.pisNumber) {
        ...
        try {
            let alreadyThisNisResponse = await api.post('/hcm/payroll/queries/personAlreadyThisNis',nisBody);
  ...
  //Caso todas as validaões passem
